...
import { fadeInLeft, scaleIn, hoverLift, tapScale } from '../../lib/animations'
      ...
      <div className="p-6">
        ...
        <div className="space-y-8">
          {roadmap.map((phase, phaseIdx) => (
            <div key={phase.phase}>
              {/* Phase header */}
              <AnimatedSection variants={fadeInLeft} className="flex items-center gap-3 mb-4">
              ...
              {/* Nodes */}
              <AnimatedGroup
                className={`grid grid-cols-1 md:grid-cols-2 gap-4 pl-4 border-l-2 ${phase.borderColor}`}
                stagger={0.1}
              >
                {phase.nodes.map((node, nodeIdx) => {
                  ...
                  return (
                    <AnimatedItem key={node.id}>
                      <motion.div
                        className={`border rounded-xl p-5 cursor-default ${node.status === 'locked' ? 'border-hairline opacity-60' : 'border-hairline'} ${isHovered && node.status !== 'locked' ? 'shadow-elevation-2' : ''}`}
                        initial="rest"
                        whileHover={node.status !== 'locked' ? 'hover' : 'rest'}
                        variants={hoverLift}
        ...
        {/* Completion reward */}
        <AnimatedSection className="mt-10" variants={scaleIn}>
          <div className="bg-block-navy text-white rounded-xl p-6 text-center">
