            ...
            {searchResults.length === 0 ? (
              ...
              <div className="space-y-2">
                {searchResults.map((item, i) => (
                  <div key={i} className="border border-hairline rounded-lg overflow-hidden">
                    <button
                    ...
                    >
                      ...
                      <ChevronDown size={16} className={`flex-shrink-0 text-ink/40 transition-transform ${openIndex === i ? 'rotate-180' : ''}`} />
                    </button>
                    {openIndex === i && (
                      <div className="px-5 pb-4 text-sm text-ink/60 leading-relaxed border-t border-hairline pt-3">{item.a}</div>
                    )}
                  </div>
                ))}
              </div>
